...
/*
function getProdDet($dom, $prod_url)
{
	global $header;
	$dat = $dom->find('#productTitle');
	$prodName = trim($dat[0]->plaintext);

	$cat = getCat($dom);
	$weight = getWeight($dom);
	$description = getProdDesc($dom);
	$brand = getBrand($dom);
	$price = getPrice($dom);
	//$shortDesc = getShortDesc($dom);
	$images = getImages($dom, $prodName);
	$bullets = getBullets($dom);
	


	// Subcategory, Title, Brand, Price, Description, Image Url 1, Image Url 2, Image Url 3, Image Url 4, Bullet Point 1, Bullet Point 2, Bullet Point 3, Bullet Point 4, Bullet Point 5, Weight, Product url


	$data = ["Subcategory"=>$cat, "Title"=>$prodName, "Brand"=>$brand, "Price"=>$price, "Description"=>$description, "Image Url 1"=>$images[0], "Image Url 2"=>$images[1], "Image Url 3"=>$images[2], "Image Url 4"=>$images[3], "Bullet Point 1"=>$bullets[1], "Bullet Point 2"=>$bullets[2], "Bullet Point 3"=>$bullets[3], "Bullet Point 4"=>$bullets[4], "Bullet Point 5"=>$bullet5, "Weight"=>$weight, "Product url"=>$prod_url ];
...
function getCategories1($dom)
{
	/*
	$expl = explode("<div", $dom);
	$cdata = substr($expl[0], 10);
	$data = json_decode($cdata, true);
	*/

	foreach ($dom->find('a') as $e) {
		$cat[] = array("id"=>$e->id, "url"=>str_replace("#", "", $e->href), "name"=>trim($e->plaintext));
	}
	return $cat;
}
...
/************/
function insertProduct($name, $price, $description, $categories, $filters)
{
	global $conn;
	global $silent;
	$query = "INSERT INTO products (name, price, description, categories, filters) VALUES ('$name', '$price', '$description', '$categories', '$filters' )";
	$conn->query($query);
	if($conn->affected_rows > 0) 
		{
...
function insertProdFilter($prod_name, $filter_id)
{
	global $conn;
	global $silent;
	$query = "UPDATE products SET filters = CONCAT(filters, ',$filter_id') WHERE name = '$prod_name'";
	$conn->query($query);
	if($conn->affected_rows > 0) 
		{
...
//////////////////////////////////////////////// Special /////////////////////////////////////////////////////////////////
function insertCat_($name, $code, $parent)  
{
	global $conn;
	global $silent;
	$query = "INSERT INTO categories (name, parent, cat_code) SELECT '$name', id, '$code' FROM categories WHERE cat_code='$parent'";
	$conn->query($query);
	if($conn->affected_rows > 0)
		{
			if(!$silent) echo "Category ($name) inserted to db **\n";
		...
		if($counter > 0)
		{
			...
			foreach ($e->find('.subcat') as $e2) {
				...
				if($cat)
				{
					...
					insertCat_($cat, $cat_code, $parent);
				}
				...
				foreach ($e2->find('li a') as $dd) {
					...
					$sub_cat = $dd->plaintext;
					$subcat_code = str_replace("#", "", $dd->href);
					insertCat_($sub_cat, $subcat_code, $parent2);
...
function processProd($id)
{
	...
	//getDom_("http://www.mcmaster.com/#".$id, true);
	$dom = getDom9_($url, false);
...
$all_categories = getCatList();
$counter = 0;
foreach ($all_categories as $cat_code) {
	
	// get the search id
	$search_id = getSearchId($cat_code);
	//echo $cat_code."\n";
	//echo $search_id."\n";

	$dom_u = searchType1($search_id, $cat_code);
	writeDom2File($dom_u, "sseearch");
	if($dom_u)
	{

		// First check if they have product listings
		// Not tested 
		$prod_idds = getProdList($dom_u);
		if($prod_idds != null)
		{
			foreach ($prod_idds as $prod_idd) {
				echo "Pid: $prod_idd\n";
				$dom_p = processProd($prod_idd); 
				writeDom2File($dom_p, "product");
			}
		}
		// End of  not tested 


		// Next check if it has the abbrid
		$abbrids = getAbbrIds($dom_u);
		$abbrname = removeCarriage($dom_u->find('.AbbrPrsnttn_PrsnttnNm', 0)->plaintext); //die();
		if($abbrname) insertCat_($abbrname, $abbrname, $cat_code);

		if($abbrids != null)
		{
			foreach ($abbrids as $abbrid) {
				$dom_a = withAbbrNext($search_id, $abbrid);
				writeDom2File($dom_a, "abbr");

				// Checking for product list again
				$prod_idd2s = getProdList($dom_a);
				if($prod_idd2s != null)
				{
					foreach ($prod_idd2s as $prod_idd2) {
						echo "Pid: $prod_idd2\n";
						$dom_p2 = processProd($prod_idd2); 

						writeDom2File($dom_p2, "product2");
						$_products = getProdDet($dom_p2);
