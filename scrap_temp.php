...
function insertProdFilter($prod_name, $filter_id_arr)
{
	...
	if($rs->num_rows > 0)
	{
		...
		if($conn->affected_rows > 0) 
		{
			if(!$silent) echo "Product Filter ($prod_name ==> $ff) inserted to db\n";
			return true;
		}
...
$query = "SELECT * FROM categories WHERE process = 'yes' AND child IS NULL";
$rs = $conn->query($query);
...
//while ($row = ['name'=>'Machine Keys', 'cat_code'=>'machine-keys'])
{
	...
	if($innerCats)
	{
		foreach ($innerCats as $innerCat) {
			...
			$catNoLinks = getCatNoLink($dom1a);
			if($catNoLinks)
			{
				foreach ($catNoLinks as $catNoLink) {
					$catNoLinkId = $catNoLink['id'];
					$catNoLinkName = $catNoLink['name'];
					$cat_db_id = insertCat_($catNoLinkName, str_replace(" ", "-", $catNoLinkName), $innerCatCode, "yes");
	...
	$catNoLinks = getCatNoLink($dom1);
	if($catNoLinks)
	{
		foreach ($catNoLinks as $catNoLink) {
			$catNoLinkId = $catNoLink['id'];
			$catNoLinkName = $catNoLink['name'];
			$cat_db_id = insertCat_($catNoLinkName, str_replace(" ", "-", $catNoLinkName), $cat_code, "yes");
